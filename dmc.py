...
class DMCApp(QWidget):
    ...
    def enqueue_playlist(self, playlist_url, choice, destination):
        try:
            ...
            api_key = "AIzaSyDp-G9Ib3j26ESPObM783IbbYZU1TuZEHI"
            api_url = f"https://www.googleapis.com/youtube/v3/playlistItems?part=id,snippet&playlistId={playlist_id}&maxResults=50&key={api_key}"
            video_ids = []
            ...
            while True:
                ...
                if "error" in data and data["error"]["code"] == 403:
                    # Quota limit exceeded, wait for 1 hour and retry
                    print("Quota limit exceeded, waiting for 1 hour...")
                    time.sleep(3600)
                    continue
                ...
                if "nextPageToken" in data:
                    ...
                    api_url = f"https://www.googleapis.com/youtube/v3/playlistItems?part=id,snippet&playlistId={playlist_id}&maxResults=50&pageToken={next_page_token}&key={api_key}"
                else:
                    break
            ...
            for video_id in video_ids:
                ...
                self.download_queue.append((stream, choice, destination))
...
def download_audio_from_playlist(playlist_url, destination):
    try:
...
download_audio_from_playlist(playlist_url, destination)
